...
For scream detection, instead of training a model from scratch, we used a pretrained model called YAMNet. YAMNet is already trained on AudioSet, which has millions of audio clips and can recognize 521 different sound classes (like human voices, animals, environmental sounds, etc.). The idea was to use YAMNet as a feature extractor. It converts audio into embeddings (basically a compressed representation of the sound that still keeps important features).

We collected around 3,200 audio samples, split evenly between scream and non-scream. Our pipeline was pretty simple: Audio → YAMNet → 1024-d embeddings → custom classifier. On top of those embeddings, we trained our own neural network for binary classification (scream vs non-scream). We ended up getting around 83% accuracy on unseen test data.
